"""
Multi-task environment wrapper for ReW-MPC.
...
class MultitaskWrapper(gym.Wrapper):
	"""Wraps a list of single-task envs into a unified multi-task interface."""
	...
	def _pad_obs(self, obs):
		if obs.shape != self._obs_shape:
			obs = torch.cat((obs, torch.zeros(
				self._obs_shape[0] - obs.shape[0], dtype=obs.dtype, device=obs.device
			)))
		return obs
